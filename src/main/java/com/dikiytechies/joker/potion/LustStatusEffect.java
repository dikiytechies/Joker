    ...
    public void applyEffectTick(LivingEntity livingEntity, int amplifier) {
        if (!livingEntity.level.isClientSide()) {
            livingEntity.level.getEntitiesOfClass(LivingEntity.class, livingEntity.getBoundingBox().inflate(1.25),
                ...
                float healthDifference = negative * Math.max(livingEntity.getHealth(), entity.getHealth()) / Math.min(livingEntity.getHealth(), entity.getHealth());
                final float damageHealAmount = (float) Math.pow(healthDifference, 3) / 160.0f * (amplifier + 1);
                if (negative == 1) {
                    entity.hurt(DamageSource.thorns(livingEntity).bypassInvul().bypassArmor().bypassMagic(), Math.min(4.5f * (amplifier + 1) + Math.abs(damageHealAmount), entity.getMaxHealth() * 0.6f));
                    livingEntity.setHealth(0.12f * (amplifier + 1) + livingEntity.getHealth());
                    ...
                    livingEntity.setHealth(0.12f * (amplifier + 1) + livingEntity.getHealth() + Math.abs(damageHealAmount));
                    entity.hurt(DamageSource.thorns(livingEntity).bypassInvul().bypassArmor().bypassMagic(), 4.5f * (amplifier + 1));
                }
            });
        }
    }
